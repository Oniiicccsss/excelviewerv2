...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file'])) {
    ...
    try {
        // --- 1. Load Spreadsheet (MODIFIED USING EXPLICIT READER) ---
        // 1. Instantiate the specific reader (assuming the uploaded file is XLSX)
        $reader = new Xlsx(); 
        
        // 2. Explicitly set the read option to ignore empty cells
        $reader->setReadEmptyCells(false); 
        
        // 3. Load the file using the configured reader
        ...
        // --- 3. Column Widths (Simplified for speed) ---
        $colWidths = [];
        $minWidth = 60; 
        $widthFactor = 8; 
        for ($c = 1; $c <= $colCount; $c++) {
